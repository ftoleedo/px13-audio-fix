...
# keep a policy the user set (see PX13_RECOVER_POLICY in px13-soundwire-recover.sh)
POLICY="$(px13_cache_get PX13_RECOVER_POLICY 2>/dev/null || true)"
{ printf '# generated by px13-audio-fix on %s - edit only PX13_RECOVER_POLICY\nPX13_ACP_PCI=%s\nPX13_CARD_LONGNAME=%s\n' \
    "$(date '+%F %T')" "$PCI" "$LONG"
  [ -n "$POLICY" ] && printf 'PX13_RECOVER_POLICY=%s\n' "$POLICY" || true
} | root_run tee /etc/px13-audio-fix.conf >/dev/null
echo "    cached: ACP PCI=${PCI:-?} -> /etc/px13-audio-fix.conf"

# The rt721-sdca jack codec shares the bus with the amps. On 7.3.0-rc2 it
# runtime-suspends ~7 s after probe and never resumes (-ENODATA on the
# regcache sync), and because PipeWire's ACP needs every mapping of the UCM
# HiFi verb to probe, that one dead codec drops the whole profile - internal
# Speaker included, no sink at all. Forbid its runtime PM at device-add time so
# the first suspend never happens. Only installed when such a codec exists.
RULE=90-px13-rt721-no-autosuspend.rules
if ls -d /sys/bus/soundwire/devices/sdw:*:025d:0721:* >/dev/null 2>&1; then
  root_run install -Dm644 "$REPO/$RULE" "/etc/udev/rules.d/$RULE"
  root_run udevadm control --reload
  for d in /sys/bus/soundwire/devices/sdw:*:025d:0721:*; do
    echo on | root_run tee "$d/power/control" >/dev/null
  done
  echo "    rt721 jack codec: runtime suspend forbidden (udev rule + applied now)"
else
  echo "    no rt721 jack codec on the bus - udev rule not needed"
fi
