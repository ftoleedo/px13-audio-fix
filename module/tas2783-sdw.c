...
/*
 * sdw_slave_wait_for_init() is a static inline in <linux/soundwire/sdw.h>
 * from 7.2 on. Whether THIS kernel's header has it is probed by the Makefile
 * (HAVE_SDW_SLAVE_WAIT_FOR_INIT) rather than guessed from LINUX_VERSION_CODE,
 * because distros backport: two trees with the same version code can differ.
 * Carry a copy when it is missing so the resume path is identical everywhere.
 */
#ifndef HAVE_SDW_SLAVE_WAIT_FOR_INIT
			 ...
			 const struct sdw_device_id *id)
{
	...
	if (i >= 0 && i < peripheral->sdca_data.num_functions) {
		 ...
		 * function descriptor and the sdw_slave as arguments, 7.2 reads
		 * the descriptor from function_data->desc, 7.3 also dropped the
		 * sdw_slave. Which shape THIS kernel has is probed from its header
		 * by the Makefile (PX13_SDCA_PARSE_HAS_*), not from the version.
		 */
#if defined(PX13_SDCA_PARSE_HAS_DESC)
		ret = sdca_parse_function(dev, peripheral, function_data->desc,
					  function_data);
#elif defined(PX13_SDCA_PARSE_HAS_SDW)
